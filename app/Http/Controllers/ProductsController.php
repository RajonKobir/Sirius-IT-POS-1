                    ...
                    if ($warehouses) {
                        ...
                        foreach ($warehouses as $warehouse) {
                            if ($request['is_variant'] == 'true') {
                                ...
                                foreach ($Product_variants as $Product_variants_key => $product_variant) {
                                    $product_warehouse[] = [
                                        'product_id' => $Product->id,
                                        'warehouse_id' => $warehouse,
                                        'product_variant_id' => $product_variant->id,
                                        'manage_stock' => $manage_stock,
                                        'qte' => (double)$variants[$Product_variants_key]["stock"],
                                    ];
                                }
		...
		if ($user_auth->can('products_edit')){
            ...
            if ($Product->type == 'is_variant') {
                ...
                foreach ($productsVariants as $variant) {
                    ...
                    $product_warehouse_total_qty = product_warehouse::where('product_id', $id)
                    ->where(function ($query) use ($array_warehouses_id) {
                        return $query->whereIn('warehouse_id', $array_warehouses_id);
                    })
                    ->where('deleted_at', '=', null)
                    ...
                    $variant_item['stock'] = $product_warehouse_total_qty;
                ...
                $item['stock'] = $product_warehouse_total_qty;
