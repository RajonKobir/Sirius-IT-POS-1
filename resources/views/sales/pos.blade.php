...
<head>
  ...
  <title>{{ env('APP_NAME', 'Sirius IT Consultancy');  }}</title>
            ...
            this.$refs.create_pos.validate().then(success => {
              if (!success) {
                ...
                if (this.sale.client_id == "" || this.sale.client_id === null) {
                  toastr.error('{{ __('translate.Choose_Customer') }}');
         ...
         Submit_Payment() {
          ...
          Verified_paidAmount() {
              if (isNaN(this.payment.montant)) {
                  ...
                  toastr.warning('{{ __('translate.Paying_amount_is_greater_than_Grand_Total') }}');
                  this.payment.montant = 0;
              } 
          },
              ...
              axios.post("/pos/create_pos", {
                ...
                .then(response => {
                  if (response.data.success === true) {
                    ...
                    toastr.success('Please allow the popup for invoice! This page will be reloaded within 10 seconds!');
                    toastr.success('{{ __('translate.Created_in_successfully') }}');
                    window.open("/invoice_pos/" + response.data.id, "_blank");
                    setTimeout(delaySeveralSeconds, 10000);
                    function delaySeveralSeconds(){
